...
-   V závislosti na rozlišení, pro přehrávání videa může být potřeba
    > větší grafická paměť a výkonnější grafický adaptér

-   Dotykové ovládání -- vyžaduje dotykovou obrazovku

-   Některá média DVD/CD můžou potřebovat ke svému načtení s nimi
    > kompatibilní mechaniky

-   BitLocker potřebuje Trusted Platform Module (TPM) 1.2

-   Hyper-V -- potřebuje další min. 2GB RAM, 64bitový procesor se
    > zapnutou virtualizací (Intel VT nebo AMD-V) a podporou SLAT
...
$w11_network = @(
    New-LabNetworkAdapterDefinition -UseDhcp -InterfaceName LAN0 -VirtualSwitch External
    New-LabNetworkAdapterDefinition -UseDhcp -InterfaceName LAN1 -VirtualSwitch Private1

)

Add-LabMachineDefinition -Name w11 -Memory 16GB -Processors 8 -NetworkAdapter $w11_network -OperatingSystem 'Windows 11 Pro'
Add-LabMachineDefinition -Name w11-old -Memory 16GB -Processors 8 -NetworkAdapter $w11_network -OperatingSystem 'Windows 11 Pro'
...
Invoke-LabCommand -ActivityName 'Create Users' -ScriptBlock {
    ...
    New-LocalUser -Name "$username" -Password $password -FullName "$username" -Description "Old user profile"
...
3.  Zapněte režim tabletu a předveďte celoobrazovkovou nabídku start --
    živé dlaždice i přehled aplikací.

    -   Následně vypněte režim tabletu

4.  Předveďte virtuální plochy[^8]

    -   Win+Ctrl+D pro přidání

    -   Win+Ctrl+Left nebo Right pro přepínání

5.  V režimu Plochy ukažte následující
...
6.  Předveďte Computer Management konzoli -- z nabídky pro správu i
...
> flash disku)
>
> Instalační DVD s Windows 11 `D:\LabSources\ISOs\Win11_22H2_English_x64v2.iso`

1.  Hyper-V: Zkontrolujte, že do VM **W11** je připojený prázdný disk
    (`D:\LabSources\ISOs\Win11_22H2_English_x64v2.iso`)
...
4.  Zkopírujte obsah `D:\sources` do `C:\sources`. (`install.wim` musí být na zapisovatelném disku) 
...
MetaData</td>
<td><p>Pouze pro případ, že nemáte licenční klíč, je nutno specifikovat
obraz ze souboru, viz <strong>Unattend.chm</strong>.<a href="#fn1"
...
<ol>
<li
id="fn1"><p>https://docs.microsoft.com/en-us/windows-hardware/customize/desktop/unattend/microsoft-windows-setup-imageinstall-osimage-installfrom-metadata<a
href="#fnref1" class="footnote-back" role="doc-backlink">↩︎</a></p></li>
</ol>
</section>

13. Pro automatizaci dalších fází instalace přidejte komponenty
...
14. Proveďte validaci souboru s odpověďmi, uložte.
...
9.  Zadejte příkaz:
    ...
    > `scanstate \\W11-old\share\migration /i:migdocs.xml /i:migapp.xml /o`
