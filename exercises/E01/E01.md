...
- [Úkoly](#úkoly)
  ...
  - [Lab 06 -- Windows 10 Upgrade Checker](#lab-06----windows-10-upgrade-checker)
  - [Lab 07 -- Windows 8 Upgrade Assistant](#lab-07----windows-8-upgrade-assistant)
  - [Lab 08 -- User State Migration Tool](#lab-08----user-state-migration-tool)
  - [Lab 09 -- User State Migration Tool](#lab-09----user-state-migration-tool)
...
a společnou část viz [exercises/README.md](../README.md).
...
| ----------- | ------------------------------------------------------ | ---------------------------------------- |
| **W11**     | DHCP                                                   | 192.168.0.10                             |
| **W11-OLD** | DHCP                                                   | 192.168.0.11                             |
...
> **Cíl cvičení**
>
> Seznámit se s virtualizační technologií Hyper-V.
>
> **Potřebné virtuální stroje**
>
> **W11**

1.  Zapněte Správce technologie Hyper-V

2.  Vyberte VM **W11** a zobrazte nastavení VM. Najděte, kde se
    mění:
...
2. Přidejte prázdný SCSI disk 1 GB do VM `W11` (`E:\AutomatedLab-VMs\flash.vhdx`).
   1. Naformátujte disk jako jednu partition s NTFS. (bude třeba pro `Autounattend.xml`)
...
2.  Otevřete **Rychlá nastavení** (Win+A) a **Oznamovací centrum** (Win+N), které ve
    Windows 11 nahradily Centrum akcí; porovnejte **Settings** a starý **Control Panel**

3.  Vyzkoušejte virtuální plochy[^8]
...
5.  Otevřete Computer Management konzoli -- z nabídky Win+X i
    z kontextové nabídky na **This PC** (ve Windows 11 přes *Show more options* -> **Manage**)
...
> **Cíl cvičení**
>
> Nainstalovat balík Windows Assessment and Deployment Kit (WADK)
>
> **Potřebné virtuální stroje**
>
> **W11**
...
> flash disku)
>
> Instalační DVD s Windows 11 `D:\LabSources\ISOs\win11.iso` (multi-edition ISO, obsahuje i **Windows 11 Pro**)

1.  Hyper-V: Zkontrolujte, že k VM **W11** je připojený prázdný disk
    `flash.vhdx` (viz Lab 01) a jako DVD obraz `D:\LabSources\ISOs\win11.iso`
...
4.  Zkopírujte `install.wim` z DVD (`E:\sources\install.wim`; písmeno
    jednotky DVD se ve VM může lišit) do `C:\sources`. (`install.wim` musí
    být na zapisovatelném disku)

5.  Z nabídky **Start** spusťte **Windows System Image Manager**
    (Správce bitových kopií systému Windows).

6.  V levé dolní části klikněte pravým tlačítkem myši na **Select a
...
7.  Vyberte obraz **Windows 11 Pro**

8.  Na dotaz zda, chceme vytvořit katalog, odpovězte „**Yes**"

9.  Vytvořte nový soubor odpovědí a uložte ho jako **Autounattend.xml**.

10. Do souboru odpovědí přidejte komponenty z tabulky, které za Vás
...
MetaData</td>
<td>
<p>Určuje obraz (edici) v <code>install.wim</code>, který se má instalovat.
[Key: /IMAGE/NAME Value: Windows 11 Pro]</p>
<p>(alternativně Key: /IMAGE/INDEX a číslo obrazu podle <code>dism /Get-ImageInfo</code>)</p></td>
</tr>
<tr class="even">
...
typu, FS. Pozor: aktivní oddíl může být na disku jen jeden.</td>
</tr>
</tbody>
</table>

11. Pouze pro případ, že nemáte licenční klíč, je nutno specifikovat
    obraz ze souboru (komponenta *InstallFrom | MetaData* výše), viz dokumentaci komponenty[^12]

12. Pro automatizaci dalších fází instalace přidejte komponenty
...
13. Proveďte validaci souboru s odpověďmi, uložte jako `Autounattend.xml`.

14. Přeneste vytvořený soubor s odpověďmi na cílové médium, které bude
    posléze připojené k novému PC (`flash.vhdx`).

15. Vypněte **W11**

16. Hyper-V: Odpojte disk (`flash.vhdx`) od **W11**

17. Hyper-V: Zmodifikujte `EmptyVM`

    -   připojte DVD disk `D:\LabSources\ISOs\win11.iso`
    -   připojte `flash.vhdx` obsahující soubor odpovědí
    -   a nastavte boot z (první) DVD

18. Nabootujte `EmptyVM` pomocí instalačního DVD Windows 11
    s připojeným zařízením, na němž je soubor odpovědí a proveďte
    instalaci systému (`flash.vhdx`).

19. Ověřte, že instalace probíhá automaticky (při práci offline to
    doložte snímkem obrazovky), a pak ji ukončete.

> **Pozn. (Windows 11 24H2/25H2):** přepracovaný instalátor Windows 11 od
> verze 24H2 změnil zpracování souboru odpovědí -- `AutoUnattend.xml` bývá
> vyhodnocen přísněji a část odpovědí (zejména sekce `ModifyPartitions`,
> fáze *specialize*/*oobeSystem*) může být ignorována, takže se objeví
> dialogy instalátoru (např. *Select language settings*), přestože je soubor
> odpovědí připojen. Ověřeno na stanicích C304 (Windows 11 25H2). Jde o
> známou změnu v instalátoru MS; cvičení WSIM (tvorba a validace souboru
> odpovědí) tím není dotčeno, ale plně bezobslužná instalace na 24H2/25H2
> může vyžadovat úpravy (viz diskuse komunity, např. elevenforum.com).
...
## Lab 06 -- Windows 10 Upgrade Checker

Podívejte se do dokumentace jak vypadá Windows 10 Upgrade Checker a jaké informace Vám přináší.

## Lab 07 -- Windows 8 Upgrade Assistant

Podívejte se do dokumentace jak vypadá  Windows 8 Upgrade Assistant a jaké informace Vám přináší.

> Pozn.: současnou obdobou těchto historických nástrojů je pro Windows 11
> aplikace [PC Health Check](https://aka.ms/GetPCHealthCheckApp), která
> kontroluje hardwarové požadavky Windows 11.

## Lab 08 -- User State Migration Tool
...
## Lab 09 -- User State Migration Tool
...
11. Spusťte příkazový řádek jako administrátor a přejděte do
