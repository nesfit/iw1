...
> **Cíl cvičení**
>
> Nainstalovat balík Windows Assessment and Deployment Kit (WADK), se
> kterým budete pracovat ve všech dalších úkolech.
>
...
4.  Ze stejné stránky stáhněte a nainstalujte [Windows PE add-on for the Windows ADK](https://go.microsoft.com/fwlink/?linkid=2289981).
...
3.  Vytvořte vlastní WinPE ISO obraz s integrovaným nástrojem **ImageX**
    ...
    c.  `copy "C:\Program Files (x86)\Windows Kits\10\Assessment and Deployment Kit\Deployment Tools\amd64\DISM\imagex.exe" D:\winpe\mount\Windows\System32\`

        - pozor na verzi **amd64** -- 32bitový `imagex.exe` by v 64bitovém
          WinPE (bez WoW64) nešel spustit

    d.  `dism /Unmount-Image /MountDir:D:\winpe\mount /Commit`

    e.  `makewinpemedia /iso D:\winpe D:\winpe\winpe-imagex-amd64.iso`

4.  Nasdílejte složku `D:\LabSources\ISOs` na hostovském počítači jako `ISOs` s právy `Full Control` pro `Everyone`.
5.  Zkopírujte `D:\winpe\winpe-imagex-amd64.iso` do `\\hXY\ISOs` (XY je číslo
    Vašeho PC; jde o složku `D:\LabSources\ISOs` hostitele nasdílenou v předchozím
    kroku). Nepřepisujte předpřipravená `winpe-w10-amd64.iso` / `winpe-w11-amd64.iso`.
...
1.  V Hyper-V Manager přidejte `DVD Drive` do `W11` s obrazem `D:\LabSources\ISOs\win11.iso`.
2.  Zkopírujte `E:\sources\install.wim` do `D:\sources` (`E:` je jednotka DVD ve VM; písmeno se může lišit)
...
5.  Nyní bitovou kopii `Windows 11 Pro` z WIM souboru připojte do
    ...
    a.  Vytvořte adresáře `D:\Deployment\mount` a `D:\scratch`

    b.  `dism /Mount-Image /ImageFile:"D:\sources\install.wim" /index:6 /MountDir:"D:\Deployment\mount" /ScratchDir:"D:\scratch"`

        - index 6 je obraz *Windows 11 Pro* v aktuálním `win11.iso`; ověřte
          podle výstupu kroku 4
...
6.  Nyní provedeme integraci ovladačů a pak aktualizací (včetně
    ...
    b.  `dism /Image:"D:\Deployment\mount" /Add-Driver /Driver:"C:\Windows\INF\3ware.inf"`

    c.  `dism /Image:"D:\Deployment\mount" /Add-Driver /Driver:"C:\Windows\INF\1394.inf"`
...
>
> **Další prerekvizity**
> Sdílená složka **\\\\W11\\Deployment** na **W11** (z úkolu Lab 03).
>
> Připravené ISO s Windows PE: vlastní z úkolu Lab 02
> (`D:\LabSources\ISOs\winpe-imagex-amd64.iso`), případně předpřipravené
> `D:\LabSources\ISOs\winpe-w11-amd64.iso`
...
4.  Připojte vytvořené ISO s Windows PE
    (`D:\LabSources\ISOs\winpe-imagex-amd64.iso`) k **W11-SOURCE** a zvolte
    start tohoto disku.
...
1.  Pro zachycení bitové kopie stanice spusťte následující příkaz
    (**C:** zde představuje systémový disk; písmena jednotek se ve WinPE
    mohou lišit, ověřte je příkazem `diskpart` -> `list volume`):
...
2.  Po dokončení zachycování připojte sdílenou složku
    ...
    -   Pokud WinPE název **W11** nepřeloží, použijte místo něj IP adresu,
        kterou má W11 na adaptéru LAN0 (zjistíte ji na W11 příkazem
        `ipconfig`; WinPE dostane adresu ze stejné NAT sítě *Default Switch*)
...
> oddílů a aplikovat na něj upravenou bitovou kopii z předchozích úkolů.
>
> **Potřebné virtuální stroje**
>
> **W11**
...
>
> Připravené ISO s Windows PE (viz Lab 02)
> `D:\LabSources\ISOs\winpe-imagex-amd64.iso` (nebo předpřipravené
> `D:\LabSources\ISOs\winpe-w11-amd64.iso`)

1.  Nabootujte **W11-SOURCE** pomocí Windows PE (viz Lab 04)
...
3.  Připojte síťový disk a aplikujte **MyNewInstall.wim**:
    ...
    -   Při dotazu na uživatelské jméno použijte **W11\root** a heslo
        **root4Lab** (při potížích se jménem **W11** viz poznámka v Lab 04)
