...
Funkce Vzdálená pomoc je technologie v systémech MS Windows (Tato funkce 
je dostupná pouze u Windows verzí Pro, enterprise a education), která
...
# AutomatedLab na stanicích vypíná UAC; Lab 05 (Remote Assistance) potřebuje výchozí chování UAC na W11-1
...
| **W11-2**   | DHCP                                                   | 192.168.0.11 (brána 192.168.0.10)        |
| **W11-3**   | DHCP                                                   | 192.168.0.12 (brána 192.168.0.10)        |
...
-   Skript na **W11-1** znovu zapíná UAC (AutomatedLab je standardně
    vypíná) -- Lab 05 na této stanici výchozí chování UAC potřebuje.
...
-   Vyzkoušejte si, jak vypnout a zapnout firewall, a projděte si základní nastavení Windows Firewall.
-   Všimněte si (případně vyzkoušejte), že síť je ve Windows 11 (resp. od Windows Vista) během startu opravdu vypnutá, kdežto v XP ne.

Síť:

-   Vyzkoušejte si použití **ping**, **tracert, ipconfig** (z W11-1).

-   Podívejte se do nastavení Sítě, kde zjistíte IP adresu.

-   Otevřete **Network and Sharing Center** -- přes
     **Control Panel** (Síť a internet), případně příkazem
...
-   V **Network and Sharing Center** si projděte možnosti nastavení
     síťových adaptérů. Na adaptéru LAN1 si vyzkoušejte, jak se nastavuje
     statická IP adresa, jak se nastaví, aby počítač získával IP z DHCP
     a když není DHCP k dispozici, tak jak se nastaví APIPA nebo
     alternativní konfigurace (případně `netsh`).
...
-   Vytvořte sdílený adresář **C:\\Share** na **W11-1**; budete jej potřebovat v Lab 05.
...
5.  Otevřete **Network and Sharing Center** a nastavte odpovídající
    ...
    **Public** (viz Lab 02, pokud nemá adaptér nastavenou výchozí
...
15. Ukončete příkaz ping pomocí <kbd>Ctrl</kbd>+<kbd>C</kbd>.
...
3.  Otevřete **Settings** \ **System** \ **Remote Desktop** (ve Windows 11
    už nastavení není ve vlastnostech počítače; klasický dialog spustíte
    příkazem `SystemPropertiesRemote.exe`).

4.  Zapněte přepínač **Remote Desktop** a potvrďte.
...
9.  Připojte se k počítači **W11-1** pomocí IP adresy nebo jména
    počítače. Použijte účet **root** (W11-1\root) a heslo **root4Lab**.

10. Prozkoumejte chybu certifikátu a informace o certifikátu (**View
    certificate**) a potvrďte **Yes**.

11. Všimněte si, že došlo k přerušení sezení na stanici W11-1 a systém
    je nyní ovládán ze stanice W11-2.
...
3.  Spusťte klasický dialog vzdáleného přístupu příkazem
    `SystemPropertiesRemote.exe` (sekce *Remote Assistance*; ve Windows 11
    už není ve vlastnostech počítače).

     (*alternativně*: v nabídce Start vyhledejte „remote assistance" a
...
6.  V průvodci volte následující možnosti:
    ...
    -   Pozvánka je xml soubor s koncovkou .msrcIncident a můžete si ji
        prohlédnout pomocí textového editoru, např. poznámkového bloku.
...
14. Pokuste se z **W11-2** spustit příkazovou řádku jako
    **Administrator**. Nyní se výzva zobrazí oběma a i uživatel, který
...
-   Předveďte lektorovi s využitím stanic **W11-1**, **W11-3** a
    **W11-2** (při vypracování offline doložte snímky obrazovky v PDF reportu).
...
-   Na začátku je potřeba (vhodné):
    ...
    -   Vypnout **VM**, ideálně vrátit snímek, aby studentům
         ...
         připojení jednotlivých adaptérů podle Lab 01.
