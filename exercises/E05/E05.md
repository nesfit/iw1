...
- [**Studentské úkoly**](#studentské-úkoly)
  ...
  - [**Lab LS07 -- Prokládaný svazek (Striped Volume)**](#lab-ls07----prokládaný-svazek-striped-volume)
...
# AutomatedLab na stanicích vypíná UAC; cvičení LS01/LS02 potřebují UAC zapnuté (změna vyžaduje restart)
...
| **W11-domain** | 192.168.0.21 (DNS 192.168.0.20)          | člen domény; edice **Education** (AppLocker)   |
...
-   **W11-domain** je edice *Windows 11 Education*, protože AppLocker
    (Lab LS04) není v edici Pro podporovaný.
...
10. Zakažte položku User Account Control: Switch to secure desktop when
...
> omezení softwaru.
>
> :warning: Zastaralé (*deprecated*) od Windows 10 verze 1803. Pouze si přečtěte návod, ale nezkoušejte.
> **Potřebné virtuální stroje**
>
> **W11-1**
...
7.  Zavřete Editor místních zásad skupiny a spusťte příkaz `gpupdate /force`. Odhlaste se a znova přihlaste a pokuste se spustit
      **Poznámkový blok**.
    - **Poznámkový blok** půjde spustit, protože Software Restriction Policies byly deaktivovány od W10 1803.
...
19. Přihlaste se na **W11-domain** pod účtem **testing\\Bart**.
    ...
    2.  Přidejte DNS server s adresou **192.168.0.20** (stanice W2022-dc) na rozhraní **LAN1**.
...
27. Přihlaste se na **W2022-dc** pod účtem **testing\\root**.
...
5.  Z kontextové nabídky nově přidaných disků zvolte Inicializace disků
...
6.  Konvertujte disky 1 a 2 na dynamické. Klikněte pravým tlačítkem myši
    ...
    -   **Pozn.:** dynamické disky jsou od Windows 11 zastaralé (Microsoft
        doporučuje místo nich **Prostory úložišť**, viz LS10). Volba
        *Převést na dynamický disk* může být v Disk Managementu ve Windows
        11 25H2 nedostupná (šedá). Pokud tomu tak je, převeďte disky 1 a 2
        stejně jako disk 3 v kroku 7 pomocí `diskpart`
        (`select disk 1` -> `convert dynamic`, poté `select disk 2` ->
        `convert dynamic`) -- na stanicích C304 tato cesta funguje.
...
> **Další prerekvizity**
>
> Splnění LS05 (tři připojené disky).
...
## **Lab LS07 -- Prokládaný svazek (Striped Volume)**
...
10. V Hyper-V manageru vytvořte pevný disk o velikosti 10GB umístěný v `E:\AutomatedLab-VMs` a připojte ho k této VM.
...
-   Stáhněte aplikaci [**Process
      ...
      Zajistěte, aby ji mohl spouštět pouze
