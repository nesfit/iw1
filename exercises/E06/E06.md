...
- [**Úkoly**](#úkoly)
  ...
  - [**Lab 04 -- Zasílaní událostí**](#lab-04----zasílaní-událostí)
  ...
  - [**Lab 08 -- Nastavení odesílaných informací o chybách programů**](#lab-08----nastavení-odesílaných-informací-o-chybách-programů)
...
# pokud tam chybí, stáhne se z GitHubu (stanice má internet).
$utils = 'D:\LabSources\SoftwarePackages\IW1\E06\utils'
if (-not (Test-Path -Path "$utils\fsaTemplate.xml")) {
    New-Item -Path $utils -ItemType Directory -Force | Out-Null
    foreach ($file in 'Crash.exe', 'Malloc.exe', 'malloc.vbs', 'send_nefs_mail.vbs', 'simulate_workload.vbs', 'fsaTemplate.xml') {
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/nesfit/iw1/main/exercises/E06/utils/$file" -OutFile "$utils\$file" -UseBasicParsing
    }
}
...
hostitelské stanici). Přihlášení do VM: lokálně **root** / **root4Lab**,
...
-   Stanice tohoto labu mají jediný adaptér (LAN1) a **nemají přístup na
    ...
    na stanicích C304). Úkolu Lab 03 se týká poznámka níže.
...
> stav oznámí uživateli s informacemi o aktuálním stavu volného místa --
> v C304 zprávou pomocí programu **msg** (bod 17), mimo lab volitelně
> e-mailem (body 13--15, vyžaduje internet).
>
> **Potřebné virtuální stroje**
>
> **W11-domain**
...
>
> Pozn.: stanice labu nemají přístup na internet (viz Lab 00), odeslání
> e-mailu tedy z VM nepůjde -- použijte náhradní variantu s programem
> **msg** (bod 17), případně skript spusťte mimo lab.
>
> Skript **send_nefs_mail.vbs** (složka **C:\utils**). Nastavte heslo
...
8.  Spusťte **Prohlížeč událostí** (Event Viewer) a lokalizujte záznam
    ...
    -   Využit vlastní zobrazení **LastHourPerformanceAlerts** vytvořené
        v [**Lab 02**](#lab-02----vytvoření-vlastního-zobrazení), jenž zobrazuje události
...
20. Pod uzlem Odběry (Subscriptions) prohlížeče událostí uvidíte
 ...
 aktivním adaptéru cílové stanice W2022-dc, a případně zkuste vytvořit
 odběr znova.
...
21. Prohlédněte si výpis příkazů
    ...
    -   `wecutil gr "<názevOdběru>"` (zobrazí aktuální stav
        ...
        komunikaci - hodnota LastHeartBeatTime)
...
## **Lab 08 -- Nastavení odesílaných informací o chybách programů**
