...
<!-- TODO-SCREENSHOT: snímek 36 je historický nástroj Windows Easy Transfer z Windows 8, ten se znovu pořídit nedá -->
...
- Vydáno **29. 7. 2015**, poslední verze 22H2; do 21H1 dva feature updaty ročně, od 21H2 jeden
...
- Jediná edice s podporou **S mode** (jen aplikace z Microsoft Store)
...
| Disk                  | 16 GB / 20 GB (do 1809), od 1903 32 GB              | 64 GB                                                                   |
...
Virtuální stroje: Windows 11 lze nainstalovat jen do VM generace 2 s virtuálním TPM a Secure Boot (v Hyper-V šablona „Microsoft Windows“). Upgrade existující VM generace 1 na Windows 11 není možný. Hostitelský procesor musí také splňovat požadavky Windows 11. Laboratorní stroje z AutomatedLab (E01) jsou generace 2; vTPM a Secure Boot (šablona Microsoft Windows) jim zapíná až parametr -HyperVProperties @{ EnableTpm = 'true'; EnableSecureBoot = 'on'; SecureBootTemplate = 'MicrosoftWindows' } ve skriptu Lab 00 – AutomatedLab totiž aplikuje WIM offline a požadavek na TPM sám nekontroluje (bez parametru vznikne VM bez vTPM, Windows 11 přesto běží). Zdroj: https://automatedlab.org/en/stable/AutomatedLabDefinition/en-us/Add-LabMachineDefinition/
...
Zdroj (Windows 10 od verze 1903 vyžaduje 32 GB pro obě architektury): https://support.microsoft.com/en-us/windows/windows-10-system-requirements-6d4e9a79-66bf-7950-467c-795cf0386715 -->
...
- **Windows Deployment Services** (WDS) – role Windows Server
  - PXE boot, multicast; volitelně integrovaný s AD DS, nebo režim *Standalone* (bez domény, od Windows Server 2012)
...
V laboratoři (E01, Lab 03) se instaluje Deployment Tools + USMT; Lab 04 vytváří Autounattend.xml pro Windows 11 Pro s generickým instalačním klíčem VK7JG-NPHTM-C97JM-9MPGT-3V66T (retail kanál – jen vybere edici Pro, není to GVLK; GVLK Pro pro KMS je W269N-WFGWX-YVC9B-4J6C9-T83GX).
...
Snímek z Windows 11 24H2 (Windows SIM z ADK 10.1.26100.2454, katalog install_Windows 11 Pro.clg vygenerovaný z install.wim v C:\sources) s otevřeným souborem Autounattend.xml z Lab 04: vlevo dole komponenty amd64_… z katalogu, uprostřed průchod 1 windowsPE (International-Core-WinPE, Setup – DiskConfiguration, ImageInstall, UserData/ProductKey), vpravo vlastnosti ProductKey (GVLK Pro, WillShowUI OnError), dole Validation bez chyb. Uživatelské rozhraní Windows SIM se od Windows 7 nezměnilo.
...
- **Přepracovaný instalátor** (SetupPrep) změnil zpracování souboru odpovědí
  - Ignorována mohou být nastavení průchodu *windowsPE* (jazyk, EULA, disk – `DiskConfiguration/ModifyPartitions`) i pozdějších průchodů *specialize*/*oobeSystem* → objeví se dialogy instalátoru (např. *Select language settings*)
  - Soubor na pevném (SCSI) disku není v pořadí hledání – hledá se na vyměnitelných médiích, v `\Sources`, `%SYSTEMDRIVE%` a na jednotce se `setup.exe`
  - Ověřeno na stanicích C304 (Windows 11 25H2, médium 24H2) – viz E01, Lab 04
- **Co dělat**
  - Rozlišit „nenalezen“ od „ignorován“: Shift+F10 → `X:\$Windows.~BT\Sources\Panther\setupact.log` (hledat *answer file*)
  - Řešení: Windows PE z ADK + `LabConfig\BypassTPMCheck` + `setup.exe /unattend:<soubor>` (odkaz *Previous version of Setup* na médiu 26100.1742 není)
  - Soubor odpovědí validovat ve Windows SIM, testovat na Hyper-V VM generace 2 s vTPM
  - Nastavení průchodů *specialize*/*oobeSystem* řešit až po nasazení bitové kopie (`Sysprep /unattend`, L02)

<!--
Dokumentace Microsoftu (Windows Setup Automation Overview, 2021) změnu chování 24H2 nepopisuje. Komunitní zdroje popisují dvě varianty: NTLite (vlákno 5166) – nový instalátor zcela ignoruje nastavení průchodu windowsPE (jazyk, EULA, disk), Shell-Setup funguje; Microsoft Q&A – přeskočeny specialize/oobeSystem. Dialog výběru jazyka tedy znamená buď ignorovaný průchod windowsPE, nebo soubor vůbec nenalezený – v E01 leží Autounattend.xml na flash.vhdx připojeném jako pevný SCSI disk, který v oficiálním pořadí hledání není (removable media, \Sources, %SYSTEMDRIVE%, jednotka setup.exe). Instalátor 24H2 volá SetupPrep.exe; postupy vkládající skripty nebo soubory přes Autounattend.xml přestaly fungovat. Ověřeno v C304 (9/2026, médium 26100.1742): nový instalátor Autounattend.xml na pevném VHDX nenačte (minimální i úplný soubor), odkaz Previous version of Setup na první obrazovce chybí (jen Support a Legal), Shift+F10 → X:\sources\setup.exe zablokuje běžící instalátor; v boot.wim 24H2 není findstr ani tasklist (protokol prohlížet přes notepad nebo type | more). Klasický instalátor spuštěný z ADK WinPE s /unattend: soubor odpovědí načte.
Windows PE z ADK nemá ovladače TPM, proto se před spuštěním instalátoru zapíše `reg add HKLM\SYSTEM\Setup\LabConfig /v BypassTPMCheck /t REG_DWORD /d 1 /f` – jinak kontrola hardwaru skončí „This PC can't run Windows 11“ i na VM s vTPM (klíč platí jen pro tento běh instalátoru). E01 Lab 04 (kroky 15–19) proto instalaci spouští takto: WinPE ISO v jediné DVD mechanice, za běhu WinPE výměna média za win11.iso, klíč LabConfig a `D:\sources\setup.exe /unattend:C:\Autounattend.xml` (klasický instalátor, splní i ModifyPartitions); `D:\setup.exe /unattend:` spustí nové UI a s klíčem LabConfig funguje také. Tvorba a validace souboru odpovědí ve Windows SIM tím dotčena není.

Zdroj (pořadí hledání souboru odpovědí): https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/windows-setup-automation-overview
Zdroj (windowsPE nastavení ignorována): https://ntlite.com/community/threads/windows-11-24h2-bypasses-the-unattend-settings-for-windows-pe.5166/
Zdroj (specialize/oobeSystem přeskočeny): https://learn.microsoft.com/en-us/answers/questions/671b06d7-7a22-48c6-aebd-4fb3cb5a06b6/
-->
...
- **KMS** (*Key Management Service*) – aktivace proti vlastnímu serveru v síti
  - Hostitel s klíčem CSVLK („KMS host key“), klienti s generickým klíčem **GVLK** (Windows 11 Pro `W269N-WFGWX-YVC9B-4J6C9-T83GX`, Enterprise `NPPR9-FWDCX-D2C8J-H872K-2YT43`)
...
V Lab 04 (E01) se používá generický instalační klíč Windows 11 Pro VK7JG-NPHTM-C97JM-9MPGT-3V66T (retail kanál – jen vybere edici, není to GVLK, KMS s ním klienta neaktivuje); systém se bez aktivace nainstaluje ve správné edici. GVLK Pro pro KMS je W269N-WFGWX-YVC9B-4J6C9-T83GX. Zdroj: https://learn.microsoft.com/en-us/windows-server/get-started/kms-client-activation-keys
...
Microsoft Store for Business/Education byl (po odkladu původního termínu 31. 3. 2023) ukončen; dokumentace uvádí, že upgrade edic přes něj už není dostupný („no longer available with the retirement of the Microsoft Store for Business“) – upgrade edic se dnes řeší subscription activation nebo klíči.
...
Dnes jsou všechny tyto cesty jen historií – Windows 7/8.1 i Windows 10 jsou bez podpory; cvičení Lab 06 v E01 používá PC Health Check a skript HardwareReadiness.ps1 (Zdroj: https://aka.ms/HWReadinessScript).
...
- **Neexportovaná data** jsou ztracena
...
V E01 zůstal k Windows Easy Transfer jen krátký historický odstavec (nástroj nelze na Windows 11 spustit). Zdroj: https://learn.microsoft.com/en-us/windows/whats-new/removed-features
-->
...
- **Vlastní konfigurační soubory** (MigXML: `<component>`, `<include>`, `<exclude>`)
...
- Příklad **migrace dat konkrétního uživatele** do sdíleného adresáře (E01, Lab 08)
...
Zdroj: https://learn.microsoft.com/en-us/windows/deployment/usmt/usmt-scanstate-syntax
Zdroj: exercises/E01/E01.md (Lab 08)
-->
...
- **Komprimované úložiště** (výchozí)
  ...
  - Ověření validity obsažených souborů a katalogu
    - `UsmtUtils /verify[:summary|all|catalog|failureonly] <úložiště-dat>`
