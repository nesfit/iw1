...
<!-- TODO-SCREENSHOT: snímky 7, 8, 9, 14, 16, 28, 35 – pořídit znovu z Windows 11 25H2 (snímek 7 je Zabezpečení Windows z Windows 10, snímky 8, 9, 14, 16 a 28 jsou dialogy z Windows 8/8.1, snímek 35 je Vzdálená pomoc z Windows 7 – ten lze nahradit snímkem Rychlé pomoci) -->
...
- Historie: Brána Windows Firewall (Vista – Windows 10 1709) → Firewall v programu Windows Defender (od Windows 10 1709/1809); dokumentace Microsoftu od roku 2024 používá opět název Windows Firewall
...
<!--
Snímek je z Windows 10, ve Windows 11 má stránka stejné rozložení (Doménová síť, Privátní síť, Veřejná síť, odkazy Povolit aplikaci v bráně firewall, Poradce při potížích, Nastavení oznámení firewallu, Upřesnit nastavení, Obnovit brány firewall na výchozí nastavení).
...
<!--
Klasický applet Ovládacích panelů (firewall.cpl) je ve Windows 11 stále k dispozici, i když Microsoft postupně přesouvá nastavení do aplikace Nastavení a Zabezpečení Windows. Ukázat stav profilů (Privátní sítě / Host nebo veřejné sítě), aktivní síť a stav oznámení.
-->
...
Snímek dialogu oznámení je z Windows 8 (Skype); ve Windows 11 má dialog „Upozornění zabezpečení Windows“ stejné volby (Privátní sítě / Veřejné sítě, Povolit přístup / Storno).
-->
...
<!--
Přehled v konzole WFAS: pro každý profil stav firewallu, výchozí akce pro příchozí (blokována) a odchozí (povolena) připojení; aktivní profil je označen. Snímek je z Windows 8, konzola ve Windows 11 vypadá stejně, jen v titulku má „Firewall v programu Windows Defender s pokročilým zabezpečením“.
-->
...
Aplikace „Remote Desktop“ z Microsoft Store přestala být podporována 27. 5. 2025 (připojení k Windows 365/AVD/Dev Box z ní jsou blokována). Datum ukončení podpory samostatného klienta Remote Desktop (MSRDC, MSI) 27. 3. 2026 pochází z blogu Windows IT Pro „Prepare for the Remote Desktop client for Windows end of support“ – stránku se nepodařilo načíst, datum bylo převzato z výsledků vyhledávání; před přednáškou ověřit.
...
<!--
Vlevo klient Připojení ke vzdálené ploše (mstsc.exe) – záložky Obecné, Zobrazení, Místní prostředky, Programy, Výkon, Upřesnit (zde ověřování serveru, RD Gateway a přihlášení webovým účtem Entra ID). Vpravo klasický dialog Vlastnosti systému > Vzdálený přístup (SystemPropertiesRemote.exe) s částmi Vzdálená pomoc a Vzdálená plocha. Oba snímky jsou z Windows 8, ve Windows 11 vypadají dialogy stejně.
...
- Využívá protokol RDP, naslouchání na portu 3389 (TCP)
...
<!--
Snímek relace Vzdálené pomoci z Windows 7 (osoba přijímající pomoc sdílí ovládání počítače). Okno ve Windows 11 vypadá prakticky stejně (msra.exe se od Windows 7 nezměnil). Při novém pořízení snímku zvážit nahrazení snímkem relace Rychlé pomoci.
-->
...
| Vzdálená pomoc systému Windows (`msra`) | RDP, TCP 3389 | Legacy, LAN / doména |
