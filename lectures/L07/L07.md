...
<!-- TODO-SCREENSHOT: snímky 4, 11, 13, 25, 38, 40, 46, 49 – pořídit znovu z Windows 11 25H2 (výzvy UAC, dialog Nastavení řízení uživatelských účtů, gpedit/secpol, Zásady hesla, SRP a AppLocker jsou z Windows 8; snímek 3 je jen ikona štítu) -->
...
- Barva záhlaví: šedá = součást Windows / podepsaný vydavatel, žlutá = nepodepsaný nebo nedůvěryhodný

<!--
Vlevo výzva k zadání souhlasu (consent prompt – správce v režimu schválení správce), vpravo výzva k zadání pověření (credential prompt – standardní uživatel). Snímky jsou z Windows 8; ve Windows 11 je dialog přepracovaný (Fluent), ale obsah je stejný: název aplikace, ověřený vydavatel, původ souboru, tlačítka Ano / Ne, u pověření volba účtu (heslo, PIN, Windows Hello, klíč zabezpečení).
Windows nejprve zjistí vydavatele souboru: Windows, ověřený (podepsaný) vydavatel, neověřený (nepodepsaný) – podle toho obarví výzvu.
Zdroj: https://learn.microsoft.com/en-us/windows/security/application-security/application-control/user-account-control/how-it-works
-->
...
→ Systém použije **skrytý, systémem spravovaný účet správce** s odděleným profilem
→ Izolovaný token správce „just-in-time“ → Správa počítače (`compmgmt.msc`)
→ Po ukončení procesu je token **zničen**
...
- Dostupnost: Windows 11 24H2 a 25H2 – volitelná aktualizace KB5120998 (27. 8. 2026), poté bezpečnostní aktualizace KB5124008 (8. 9. 2026); edice Home, Pro, Enterprise, Education
...
Historie: ohlášeno na Ignite 11/2024, testováno v Insider Canary od podzimu 2024; poprvé vydáno ve volitelné aktualizaci KB5067036 (28. 10. 2025), Google Project Zero (J. Forshaw) mezitím nahlásil 9 obcházení (všechna opravena v KB5067036 nebo následnými bulletiny; článek 26. 1. 2026). „As of 1st December 2025 the Administrator Protection feature has been disabled by Microsoft while an application compatibility issue is dealt with“ – nasazování obnoveno KB5120998 (buildy 26200.9278 / 26100.9278, „Preview“) a od 8. 9. 2026 KB5124008 (buildy .9445). KB5120998: „not classified as a formal security boundary“.
Údaj o KB5124008 pochází z poznámek k vydání citovaných tiskem (pureinfotech, 4sysops); stránka podpory nebyla ověřena přímo.
Zdroj: https://learn.microsoft.com/en-us/windows/security/application-security/application-control/administrator-protection/
Zdroj: https://support.microsoft.com/servicing/os/windows-11/2026/08/kb5120998-windows-11-24h2-25h2-update
...
![center w:440](img/s09-z-stupn-symbol-pro-obsah-13.png)

- Ovládací panely → Uživatelské účty → Změnit nastavení nástroje Řízení uživatelských účtů (`UserAccountControlSettings.exe`)

<!--
Snímek dialogu je z Windows 8, ve Windows 11 vypadá stejně (starý Win32 dialog, nemá náhradu v Nastavení). Posuvník mění současně hodnoty ConsentPromptBehaviorAdmin, PromptOnSecureDesktop a EnableLUA v HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System.
Zdroj: https://learn.microsoft.com/en-us/windows/security/application-security/application-control/user-account-control/settings-and-configuration
-->
...
- Nikdy neupozorňovat
  ...
  - Zabezpečení Windows hlásí sníženou úroveň zabezpečení; UWP aplikace z Microsoft Store se nespustí
...
Odpovídající hodnoty registru (ConsentPromptBehaviorAdmin / PromptOnSecureDesktop): 2/1, 5/1, 5/0, 0/0. Nejnižší úroveň zároveň nastaví EnableLUA=0 (vypne UAC úplně).
...
- Spuštění příkazem `gpedit.msc` (Editor místních zásad skupiny) nebo `secpol.msc` (Místní zásady zabezpečení, ve Windows 11 v Ovládací panely → Nástroje Windows)
...
![center w:600](img/s11-z-stupn-symbol-pro-obsah-13.png)
...
Snímek gpedit je z Windows 8, cesta k zásadám i jejich názvy jsou ve Windows 11 stejné.
Zdroj: https://learn.microsoft.com/en-us/windows/security/application-security/application-control/user-account-control/settings-and-configuration
-->
...
- Řízení uživatelských účtů: Spustit všechny správce v Režimu schválení správce (*Run all administrators in Admin Approval Mode*, `EnableLUA`)
  - Zakázáním dojde k vypnutí UAC pro všechny správce (Zabezpečení Windows hlásí snížené zabezpečení)
...
Registr: ConsentPromptBehaviorAdmin 0 = bez výzvy, 1 = pověření na zab. ploše, 2 = souhlas na zab. ploše, 3 = pověření, 4 = souhlas, 5 = souhlas pro ne-Windows binárky (výchozí). Laboratorní skript E05 nastavuje -ConsentPromptBehaviorAdmin 2.
...
Snímek je z Windows 8, dialog je ve Windows 11 stejný.
...
- Pokrývá .exe, .dll, skripty (.ps1 → PowerShell Constrained Language Mode), .msi, balíčky MSIX, ovladače (jen App Control)
...
<!--
Konfigurace počítače → Nastavení systému Windows → Nastavení zabezpečení → Zásady omezení softwaru; uzel je prázdný, dokud se nezvolí Nové zásady omezení softwaru. Snímek z Windows 8; ve Windows 11 gpedit uzel stále obsahuje.
-->
...
<!--
E05 Lab 04 používá pro W11-domain edici Education s odůvodněním, že Pro AppLocker nepodporuje – od KB 5024351 (2023) to neplatí, Windows 11 Pro pravidla vynucuje také (poznámka pro úpravu E05).
...
<!--
Konfigurace počítače → Nastavení systému Windows → Nastavení zabezpečení → Zásady řízení aplikací → AppLocker (stejná cesta v Editoru správy zásad skupiny na řadiči domény, cvičení E05 Lab 04). Snímek z Windows 8.
...
<!--
Vlastnosti uzlu AppLocker: karta Vynucení (Nakonfigurováno → Vynutit pravidla / Pouze audit pro každou kolekci) a karta Upřesnit (Povolit kolekci pravidel DLL). Snímky z Windows 8, dialog je ve Windows 11 stejný.
-->
