...
<!-- TODO-SCREENSHOT: snímky 4, 5, 8, 13, 28, 42, 43, 50, 53 – pořídit znovu z Windows 11 25H2 -->
...
- MMC konzole `devmgmt.msc` (též Nastavení > Bluetooth a zařízení > Zařízení > Další nastavení zařízení a tiskárny)
...
Windows 11 nabízí běžnému uživateli přehled zařízení v Nastavení > Bluetooth a zařízení (Bluetooth, tiskárny, myš, dotykové zařízení, USB); Správce zařízení zůstává nástrojem správce.
...
Dialog vlastností zařízení vypadá ve Windows 11 stejně jako na snímcích (Windows 8), jen s novým vzhledem oken.
-->
...
![center w:640](img/s08-obr-zek-7.png)
...
Snímek je z Windows 10, dialog je ve Windows 11 shodný.
-->
...
Zásada „Apply layered order of evaluation for Allow and Prevent device installation policies across all device match criteria“ (Windows 10 1809+ / Windows 11): ve výchozím stavu má každé „Zakázat“ přednost před jakýmkoliv „Povolit“; po zapnutí platí hierarchie ID instance zařízení > ID zařízení > třída zařízení > vyměnitelná zařízení, takže konkrétnější pravidlo přepíše obecnější (typický scénář: zakázat všechna USB úložiště, povolit jeden firemní disk). Microsoft ji doporučuje v praxi zapnout. Český název zásady v editoru ověřit na stanici.
...
<!--
Snímek z Windows 8 ukazuje původních 10 zásad. Ve Windows 11 jsou ve stejné složce navíc zásady pro ID instance zařízení (povolit/zakázat) a zásada „Apply layered order of evaluation for Allow and Prevent device installation policies across all device match criteria“.
-->
...
Bez parametrů se verifier spustí jako grafický průvodce.
...
- Umožňují kontrolu integrity a původu ovladače (Authenticode podpis souboru `.cat`/`.sys`)
...
diskpart a Správa disků stojí na Virtual Disk Service (VDS), kterou Microsoft postupně nahrazuje Windows Storage Management API (WMI, namespace ROOT\Microsoft\Windows\Storage) – nad ní stojí PowerShell modul Storage i Nastavení. Nové funkce (Storage Spaces, ReFS, Dev Drive) jsou dostupné jen novým API; diskpart s nimi neumí pracovat.
...
Snímek je z Windows 8, seznam zásad je ve Windows 11 shodný.
-->
...
<!--
Snímky jsou z ovládacího panelu Prostory úložišť (Windows 8), který je ve Windows 11 stále přítomen; stejný postup (výběr neformátovaných jednotek > Vytvořit fond) nabízí i nová stránka Prostory úložišť v Nastavení.
...
<!--
Při vytváření prostoru se volí název, písmeno jednotky, souborový systém (NTFS, ReFS), typ odolnosti a velikost – ta může být díky tenkému zřizování větší než kapacita fondu (na snímku 2 × 119 GB, dvoucestné zrcadlení nabízí 119 GB využitelných).
...
- Jednoduchý (bez odolnosti) (simple)
  ...
  - V Nastavení vyžaduje alespoň 2 jednotky
...
Počty jednotek podle dokumentace Microsoftu pro Nastavení Windows 11: jednoduchý 2, dvoucestné zrcadlení 2, třícestné zrcadlení 5, parita 3, duální parita 7. PowerShell (New-VirtualDisk) dovolí i jednoduchý prostor nad fondem s jediným diskem.
Zdroj: https://support.microsoft.com/en-us/windows/storage-spaces-in-windows-b6c8b540-b8d8-fb8a-e7ab-4a75ba11f9f2
-->
...
- **Historie:** Windows 7–10 nabízely 3 základní schémata (Vysoký výkon, Rovnováha, Úsporný režim); od Windows 10 1709 byla při upgradu všechna kromě Rovnováhy odstraněna
...
- **Režim napájení** – Nastavení > Systém > Napájení a baterie
...
- **Historie:** Windows 10 1709 – posuvník výkonu ve vlastnostech baterie a Spořič baterie (snímky)
...
Snímky vpravo jsou z Windows 10 (1709): posuvník „Režim napájení (na baterii)“ v rozbalovací nabídce baterie a nastavení Spořiče baterie – ve Windows 11 odpovídají rozevíracímu seznamu Režim napájení a sekci Spořič energie v Nastavení > Systém > Napájení a baterie.
...
![center w:520](img/s50-obr-zek-13.png)

<!--
Nastavení Řízení spotřeby je pod Konfigurací počítače (snímek z Windows 8, ve Windows 11 shodná struktura: Nastavení oznámení, pevného disku, režimu spánku, tlačítka, videa a zobrazení, Určit vlastní aktivní schéma napájení, Vybrat aktivní schéma napájení).
...
- `powercfg /systempowerreport` (`/spr`) – rozšířená sestava přechodů napájení včetně connected standby; nahrazuje sleepstudy
...
Sleep Study má smysl jen na zařízeních s Modern Standby; na stolním PC s S3 vrací prázdnou sestavu. /systempowerreport je novější a obecnější varianta pokrývající všechny přechody napájení; Microsoft doporučuje používat ji místo /sleepstudy.
