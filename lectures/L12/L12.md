...
![center](img/s04-windows-update.png)

<!--
Snímek z Windows 11 24H2 (Nastavení > Windows Update, stanice C304, 9/2026). Windows Update je samostatná položka v levém menu Nastavení; stránka obsahuje Vyhledat aktualizace, přepínač „Získejte nejnovější aktualizace hned, jak budou k dispozici“, Pozastavit aktualizace (volba délky pauzy po týdnech), Historie aktualizací, Upřesnit možnosti a Program Windows Insider.
-->
...
- **Aktualizace funkcí** (*feature update*) – 1× ročně (2. pololetí), mění verzi (24H2, 25H2)
  ...
  - 25H2 je *enablement package* – malý balíček, který zapne funkce už doručené do 24H2 (stejná servisní větev; build 26100 → 26200)
...
- Historie: Windows 10 do 21H1 vydával aktualizace funkcí 2× ročně v **Semi-Annual Channel** (do 1903 dělený na SAC (Targeted) a SAC; dříve Current Branch / Current Branch for Business); podpora 18 měsíců, u podzimních vydání od 1809 pro Enterprise/Education 30 měsíců

<!--
Přehled verzí a konců podpory: https://learn.microsoft.com/en-us/windows/release-health/release-information

Zdroj: https://learn.microsoft.com/en-us/windows/deployment/update/waas-overview
Zdroj: https://learn.microsoft.com/en-us/lifecycle/faq/windows (historický servis Windows 10: H1 vydání 18 měsíců, H2 vydání 30 měsíců jen Enterprise/Education, Home/Pro vždy 18)
Zdroj: https://techcommunity.microsoft.com/blog/windows-itpro-blog/windows-update-for-business-and-the-retirement-of-sac-t/339523 (SAC-T zrušen od 1903)
...
- Historie: **volba kanálu a odložení** aktualizací funkcí (365 dní) a kvality (30 dní) byly ve Windows 10 1703–1909 přímo v Nastavení; ve Windows 11 jen zásadami
...
# Upřesnit možnosti ve Windows 11

![center](img/s10-upresnit-moznosti.png)

<!--
Snímek z Windows 11 24H2 (Nastavení > Windows Update > Upřesnit možnosti). Přepínače: aktualizace pro další produkty Microsoftu, „Chci mít nejnovější verze“ (restart co nejdříve), stahování přes připojení účtované podle objemu dat, upozornění na nutný restart; rozbalovací Doba aktivního používání; níže Další možnosti – Volitelné aktualizace, Optimalizace doručení, Obnovení, Restartovat aplikace, Nakonfigurované zásady aktualizace.
Historie: ve Windows 10 1703–1909 (snímek z 1709, prosinec 2017) tu byla volba Pololetní kanál (cílený) / Pololetní kanál a odložení aktualizací funkcí o 365 a kvality o 30 dní; ve Windows 11 tyto prvky v Nastavení nejsou (jen zásadami).
-->
...
- **Doručovaný obsah**: aktualizace Windows (funkcí, kvality, ovladače), aplikace Store, definice Defenderu, Microsoft Edge, Microsoft 365 Apps, Win32 aplikace z Intune, Teams
...
![center w:540](img/s12-optimalizace-doruceni.png)

</div>
<div>

![center w:540](img/s12-optimalizace-doruceni-upresnit.png)

</div>
</div>

<!--
Snímky z Windows 11 24H2 (Nastavení > Windows Update > Upřesnit možnosti > Optimalizace doručení): vlevo Povolit stahování z jiných počítačů (zařízení v mé místní síti / zařízení na internetu a moje místní síť) a odkazy Upřesnit možnosti a Monitorování aktivity, vpravo Upřesnit možnosti Optimalizace doručení – limity šířky pásma pro stahování na pozadí/popředí absolutně (Mb/s) nebo procentem; níže na stránce (mimo výřez) jsou limity pro nahrávání a měsíční limit uploadu.
-->
...
- **Název organizace** v oznámeních u zařízení připojených k Microsoft Entra ID („Contoso vyžaduje instalaci důležitých aktualizací“)
...
![center w:540](img/s14-doba-aktivniho-pouzivani.png)

</div>
<div>

![center w:540](img/s14-doba-aktivniho-pouzivani-rucne.png)

</div>
</div>

<!--
Snímky z Windows 11 24H2 (Nastavení > Windows Update > Upřesnit možnosti): přepínač „Upozorňovat, když je nutný restart pro dokončení aktualizace“ a rozbalená Doba aktivního používání – vlevo výchozí volba Automaticky (podle skutečného používání), vpravo Ručně se zadáním začátku a konce (max. 18 hodin). Samostatná stránka Možnosti restartování z Windows 10 (naplánovat čas restartu, více oznámení) ve Windows 11 není; plánování restartu se nabízí až na stránce Windows Update, když restart čeká.
-->
...
- Postavená nad **zásadami klienta Windows Update** – přidává schvalování, plánování a ochranné mechanismy
  - **Autopatch groups** – logické skupiny zařízení (Entra ID skupiny) s nasazovacími kruhy Test → vlastní kruhy (např. Ring1–Ring3) → Last; Test a Last jsou povinné, celkem až 15 kruhů
...
Kruhy Test → First → Fast → Broad byly v původním Autopatchi (2022–23); dnešní Autopatch group má automaticky jen kruhy Test a Last („can't be removed or renamed“), mezi ně správce přidá až 13 vlastních (celkem 15 na skupinu, 300 skupin na tenant).
...
- **Požadavky**
  - **Windows 11 Enterprise 24H2** nebo novější (GA 2. 4. 2025 pro x64, Arm64 později), zařízení na poslední baseline
...
Zdroj: https://techcommunity.microsoft.com/blog/windows-itpro-blog/hotpatch-for-windows-client-now-available/4399808 (GA 2. 4. 2025, x64; Arm64 v náhledu)
...
- Windows Update stahuje přes **WinHTTP** (systémový proxy) s HTTP Range požadavky (RFC 7233) – proxy je musí podporovat
  ...
  - **Ruční nastavení**: `netsh winhttp set proxy proxy:8080`
  - **Převzetí z nastavení uživatele**: `netsh winhttp import proxy source=ie` (Nastavení > Síť a internet > *Proxy* / *Možnosti Internetu*)
...
- **Role Windows Serveru** (stále i ve Windows Server 2025) poskytující aktualizace Microsoft klientům ve firemní síti (intranetu)
  ...
  - Databáze: Windows Internal Database (WID – zastaralá) nebo SQL Server
  - Správa konzolí WSUS nebo PowerShellem (modul UpdateServices): `Get-WsusUpdate`, `Approve-WsusUpdate`
...
Zdroj: https://techcommunity.microsoft.com/blog/windows-itpro-blog/continuing-wsus-support-for-driver-synchronization/4401042 (metadata příspěvku: datePublished 7. 4. 2025; sekundární zprávy 8. 4. 2025)
...
![center](img/s23-gpedit-windows-update.png)

<!--
Snímek z Windows 11 24H2 (Editor místních zásad skupiny, české ADMX). Zásady Windows Update jsou rozdělené do podsložek: Spravovat možnosti koncového uživatele (Manage end user experience – restarty, notifikace, termíny), Spravovat aktualizace nabízené službou Windows Server Update Service (WSUS server, cílení, frekvence), Spravovat aktualizace nabízené ze služby Windows Update (odložení, pauza, volitelné aktualizace, cílová verze) a Starší verze zásad (Legacy Policies). Cesta: Konfigurace počítače > Šablony pro správu > Součásti Windows > Windows Update.
...
![center w:860](img/s29-gpedit-ie-kompatibilni-zobrazeni.png)

<!--
Snímek z Windows 11 24H2 (Editor místních zásad skupiny > Konfigurace počítače > Šablony pro správu > Součásti systému Windows > Internet Explorer > Kompatibilní zobrazení; výřez pravého panelu se seznamem zásad) – zásady IE zůstávají v ADMX i bez aplikace Internet Explorer. Kompatibilní zobrazení simulovalo chování IE 7 pro stránky psané pro starší IE: pro všechny stránky, pro intranet, podle seznamu od Microsoftu (compatibility list přes Windows Update) nebo pro stránky zadané uživatelem. V režimu IE tuto roli hraje document mode zadaný v Enterprise Mode Site List (atribut docMode).
...
![center w:860](img/s30-gpedit-ie-ochrana-osobnich-udaju.png)

<!--
Snímek z Windows 11 24H2 (Editor místních zásad skupiny > Konfigurace počítače > Šablony pro správu > Součásti systému Windows > Internet Explorer > Ochrana osobních údajů, výřez pravého panelu; před Windows 8 se sekce jmenovala InPrivate). Zásady lze nastavit na úrovni počítače i uživatele; IE umožňoval import/export filtrovaných poskytovatelů.
